...
import {
  ...
  Layers3, FileText, Plus, Trash2, Sparkles, ExternalLink,
} from 'lucide-react';
...
function CollectionEditor({ business, onChange, collection }: { business: BusinessConfig; onChange: (next: BusinessConfig) => void; collection: CollectionKey }) {
  ...
  const add = () => {
    ...
    config.fields.forEach(field => { item[field] = field === 'rating' ? 5 : ['startingPrice', 'price', 'step'].includes(field) ? 0 : ''; });
  ...
  return <section className="dev-simple-section"><div className="dev-simple-section-head"><div><h3>{config.label}</h3><p>Edit bagian yang benar-benar digunakan component.</p></div><button className="dev-simple-button" type="button" onClick={add}><Plus size={15} />Tambah</button></div><div className="dev-simple-list">{list.map((item, index) => <article className="dev-simple-card" key={item.id || index}><div className="dev-simple-card-head"><b>{item.name || item.title || item.question || `Item ${index + 1}`}</b><button className="dev-simple-danger" type="button" onClick={() => onChange({ ...business, [config.key]: list.filter((_, i) => i !== index) })}><Trash2 size={14} /></button></div><div className="dev-simple-grid">{config.fields.map(field => <TextField key={field} label={fieldLabel(field)} value={item[field] ?? ''} type={['price', 'startingPrice', 'rating', 'step'].includes(field) ? 'number' : 'text'} multiline={['description', 'comment', 'answer'].includes(field)} onChange={value => onChange({ ...business, [config.key]: updateAt(list, index, { [field]: value } as any) })} />)}</div></article>)}</div></section>;
}
...
function ContentEditor({ business, onChange }: { business: BusinessConfig; onChange: (next: BusinessConfig) => void }) {
  ...
  const patchHero = (p: Partial<HeroConfig>) => patch({ hero: { ...(business.hero || { layoutVariant: 'split' }), ...p } as HeroConfig });
  return <div className="dev-simple-stack">
    <section className="dev-simple-section"><div className="dev-simple-section-head"><div><h3>Identitas bisnis</h3><p>Isi informasi yang tampil di website.</p></div></div><div className="dev-simple-grid"><TextField label="Nama bisnis" value={business.name} onChange={v => patch({ name: String(v) })} /><TextField label="Industri" value={business.industry} onChange={v => patch({ industry: String(v) })} /><TextField label="Tagline" value={business.tagline} onChange={v => patch({ tagline: String(v) })} /><TextField label="Nomor WhatsApp" value={business.contact.whatsappNumber} onChange={v => patch({ contact: { ...business.contact, whatsappNumber: String(v) } })} /></div></section>
    <section className="dev-simple-section"><h3>Hero</h3><p>Pesan utama website dan tombol aksi.</p><div className="dev-simple-grid"><TextField label="Headline" value={business.hero?.headline || business.heroHeadline} multiline onChange={v => patchHero({ headline: String(v) })} /><TextField label="Deskripsi" value={business.hero?.description || business.heroDescription} multiline onChange={v => patchHero({ description: String(v) })} /><TextField label="Tombol utama" value={business.hero?.primaryCtaText || business.primaryCtaText} onChange={v => patchHero({ primaryCtaText: String(v) })} /><TextField label="Tombol kedua" value={business.hero?.secondaryCtaText || business.secondaryCtaText} onChange={v => patchHero({ secondaryCtaText: String(v) })} /></div></section>
    <section className="dev-simple-section"><h3>Tentang</h3><div className="dev-simple-grid"><TextField label="Paragraf 1" value={business.aboutText.p1} multiline onChange={v => patch({ aboutText: { ...business.aboutText, p1: String(v) } })} /><TextField label="Paragraf 2" value={business.aboutText.p2} multiline onChange={v => patch({ aboutText: { ...business.aboutText, p2: String(v) } })} /></div></section>
    <CollectionEditor business={business} onChange={onChange} collection="services" /><CollectionEditor business={business} onChange={onChange} collection="pricingPackages" /><CollectionEditor business={business} onChange={onChange} collection="testimonials" /><CollectionEditor business={business} onChange={onChange} collection="gallery" /><CollectionEditor business={business} onChange={onChange} collection="process" /><CollectionEditor business={business} onChange={onChange} collection="faqs" />
  </div>;
}
...
function DesignEditor({ business, onChange }: { business: BusinessConfig; onChange: (next: BusinessConfig) => void }) {
  ...
  return <div className="dev-simple-stack">
    ...
    <section className="dev-simple-section"><h3>Hero visual</h3><div className="dev-simple-grid"><TextField label="URL gambar Hero" value={hero.backgroundImageUrl || business.heroImageUrl} onChange={v => patchHero({ backgroundImageUrl: String(v), showBackgroundImageOverlay: true })} /><TextField label="Warna background" value={hero.backgroundColor || business.theme.backgroundColor} onChange={v => patchHero({ backgroundColor: String(v), gradientStyle: 'solid' })} /></div><div className="dev-simple-stack gap-2 mt-3"><Toggle label="Tampilkan trust points" value={hero.showTrustPoints !== false} onChange={value => patchHero({ showTrustPoints: value })} hint="Poin kepercayaan di bawah CTA." /><Toggle label="Tampilkan rating" value={hero.showRatingPill !== false} onChange={value => patchHero({ showRatingPill: value })} hint="Rating pelanggan pada Hero jika layout mendukung." /><Toggle label="Tampilkan gambar background" value={hero.showBackgroundImageOverlay !== false} onChange={value => patchHero({ showBackgroundImageOverlay: value })} /><Toggle label="Tampilkan floating stats" value={hero.showFloatingStats === true} onChange={value => patchHero({ showFloatingStats: value })} /></div></section>
  </div>;
}
...
function SettingsEditor({ business, onChange }: { business: BusinessConfig; onChange: (next: BusinessConfig) => void }) {
  ...
  return <div className="dev-simple-stack">
    ...
    <section className="dev-simple-section"><h3>Kontak & Lokasi</h3><div className="dev-simple-grid"><TextField label="Telepon" value={business.contact.phone} onChange={v => onChange({ ...business, contact: { ...business.contact, phone: String(v) } })} /><TextField label="Email" value={business.contact.email} onChange={v => onChange({ ...business, contact: { ...business.contact, email: String(v) } })} /><TextField label="Alamat" value={business.contact.address} multiline onChange={v => onChange({ ...business, contact: { ...business.contact, address: String(v) } })} /><TextField label="Google Maps URL" value={business.contact.googleMapsUrl} onChange={v => onChange({ ...business, contact: { ...business.contact, googleMapsUrl: String(v) } })} /></div></section>
...
export const DevPanel: React.FC<DevPanelProps> = ({ currentBusiness, onSelectBusiness }) => {
  ...
  return <>
    ...
    {open && <div className="dev-panel-overlay"><button aria-label="Tutup" onClick={() => setOpen(false)} className="absolute inset-0 bg-slate-950/40 backdrop-blur-sm" /><aside className="dev-simple-shell">
