...
export function WebsiteStudio({ onClose }: { onClose?: () => void }) {
  ...
  const generate = () => {
    update(generateBusinessConfig(form, initialBusiness));
    setTab('preview');
    ...
    <div className="studio-body">
      <aside className="studio-sidebar">
        ...
        {tab === 'generate' && <section className="studio-panel-section">
          <div className="studio-heading"><Sparkles size={18}/><div><h2>Buat website baru</h2><p>Isi data dasar. Generator memakai template dan preset yang sudah ada.</p></div></div>
          ...
          <label>Preset bisnis<select value={form.industryPresetId} onChange={e => setForm({ ...form, industryPresetId: e.target.value as IndustryPresetId })}>{industryOptions.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        ...
        {tab === 'preview' && <section className="studio-panel-section">
          <div className="studio-heading"><FileJson size={18}/><div><h2>{business.name}</h2><p>Website memakai Hero dan section existing, bukan renderer baru.</p></div></div>
          <div className="studio-summary">
            <div><span>Template</span><b>{business.templateId || 'default'}</b></div>
            <div><span>Preset</span><b>{business.industryPresetId || 'default'}</b></div>
            <div><span>Section</span><b>{Object.values(business.sections || {}).filter(Boolean).length || 'default'}</b></div>
          </div>
          <button className="studio-edit" onClick={() => setEditing(true)}><Palette size={16}/> Edit website</button>
      ...
      <main className="studio-canvas">
        <div className="studio-canvas-toolbar">
          <div><strong>Live Preview</strong><span>{business.name}</span></div>
          <div className="studio-devices">
            <button className={device === 'desktop' ? 'active' : ''} onClick={() => setDevice('desktop')}><Monitor size={15}/></button>
            <button className={device === 'tablet' ? 'active' : ''} onClick={() => setDevice('tablet')}><Tablet size={15}/></button>
            <button className={device === 'mobile' ? 'active' : ''} onClick={() => setDevice('mobile')}><Smartphone size={15}/></button>
          </div>
        </div>
