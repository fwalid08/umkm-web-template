...
const variants: HeroLayoutVariant[] = ['split','centered','background-focus','card-overlay','minimal','editorial','image-split','floating-card','spotlight','bottom-bar'];

function luminance(hex = '#0B0F19') {
  const clean = hex.replace('#','');
  if (!/^[0-9a-f]{6}$/i.test(clean)) return 0.05;
...
function resolveHero(business: BusinessConfig): HeroConfig {
  const variant = business.hero?.layoutVariant || 'split';
...
function Background({ business, hero, dark }: { business: BusinessConfig; hero: HeroConfig; dark: boolean }) {
  ...
  const imageOpacity = hero.backgroundImageOpacity ?? (dark ? .34 : .18);
  ...
  return <div className="absolute inset-0 overflow-hidden" aria-hidden="true" style={{backgroundColor:bg}}>
    ...
    {hero.texture && hero.texture !== 'none' && <div className="absolute inset-0 opacity-20" style={{opacity:hero.textureOpacity ?? .04,backgroundImage:hero.texture === 'grid' ? 'linear-gradient(rgba(127,127,127,.35) 1px, transparent 1px),linear-gradient(90deg, rgba(127,127,127,.35) 1px, transparent 1px)' : 'radial-gradient(circle, rgba(127,127,127,.45) 1px, transparent 1px)',backgroundSize:hero.texture === 'grid' ? '32px 32px' : '20px 20px'}}/>}
  </div>;
}
...
function Content({ business, hero, dark, centered=false }: { business: BusinessConfig; hero: HeroConfig; dark: boolean; centered?: boolean }) {
  ...
  return <div className={centered ? 'text-center' : 'text-left'}>
    ...
    {hero.badgeText && <div className={`mb-4 inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold ${centered ? '' : ''}`} style={{borderColor:dark ? 'rgba(255,255,255,.16)' : 'rgba(15,23,42,.10)',backgroundColor:dark ? 'rgba(255,255,255,.07)' : 'rgba(255,255,255,.78)',color:dark ? '#F8FAFC' : '#334155'}}><span className="h-1.5 w-1.5 rounded-full" style={{backgroundColor:primary}}/>{hero.badgeText}</div>}
...
export const HeroV2: React.FC<Props> = ({business}) => {
  ...
  const variant = variants.includes(hero.layoutVariant) ? hero.layoutVariant : 'split';
  ...
  if (variant === 'background-focus') return <section id="beranda" className="relative min-h-[640px] overflow-hidden"><Background business={business} hero={{...hero,backgroundType:'image-overlay'}} dark={true}/><div className="relative z-10 mx-auto flex min-h-[640px] max-w-7xl items-center px-4 py-20 sm:px-6"><Content business={business} hero={hero} dark={true}/></div></section>;
  if (variant === 'card-overlay') return <section id="beranda" className="relative min-h-[620px] overflow-hidden py-14 sm:py-20"><Background business={business} hero={{...hero,backgroundType:'image-overlay'}} dark={true}/><div className="relative z-10 mx-auto flex max-w-7xl items-center justify-center px-4 sm:px-6"><div className="w-full max-w-3xl rounded-3xl border border-white/15 bg-slate-950/70 p-6 shadow-2xl backdrop-blur-xl sm:p-10"><Content business={business} hero={hero} dark={true}/></div></div></section>;
  ...
  if (variant === 'floating-card') return <section id="beranda" className="relative min-h-[620px] overflow-hidden"><Background business={business} hero={{...hero,backgroundType:'image-overlay'}} dark={true}/><div className="relative z-10 mx-auto flex min-h-[620px] max-w-7xl items-center px-4 py-16 sm:px-6"><div className="max-w-xl rounded-3xl border border-white/15 bg-white/10 p-6 shadow-2xl backdrop-blur-xl sm:p-8"><Content business={business} hero={hero} dark={true}/></div></div></section>;
  ...
  if (variant === 'bottom-bar') return <section id="beranda" className="relative min-h-[650px] overflow-hidden"><Background business={business} hero={{...hero,backgroundType:'image-overlay'}} dark={true}/><div className="relative z-10 mx-auto flex min-h-[650px] max-w-7xl items-end px-4 py-12 sm:px-6"><div className="w-full"><Content business={business} hero={hero} dark={true}/><div className="mt-8 flex flex-wrap items-center gap-4 rounded-2xl border border-white/10 bg-black/35 p-4 backdrop-blur-md"><span className="text-sm font-semibold text-white">{hero.trustBadgeText || 'Dipercaya pelanggan'}</span>{hero.ratingValue && <span className="flex items-center gap-1 text-sm text-slate-200"><Star className="h-4 w-4 fill-current" style={{color:business.theme.accentColor}}/>{hero.ratingValue}</span>}</div></div></div></section>;
  if (variant === 'split') return <section id="beranda" className={`${sectionClass} py-16 sm:py-20 lg:py-24`}><Background business={business} hero={hero} dark={dark}/><div className="relative z-10 mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-12"><div className="lg:col-span-7"><Content business={business} hero={hero} dark={dark}/></div><div className="lg:col-span-5"><Visual business={business} hero={hero} dark={dark}/></div></div></section>;
  return <section id="beranda" className={`${sectionClass} py-16 sm:py-20`}><Background business={business} hero={hero} dark={dark}/><div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6"><Content business={business} hero={hero} dark={dark}/></div></section>;
};
